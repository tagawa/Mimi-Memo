...
assert.equal(e.schemaVersion, 3, 'schemaVersion 3');
...
// schemaVersion 3 and new fields present with null defaults
reset();
const newEp = createEpisode();
assert.equal(newEp.schemaVersion, 3, 'schemaVersion is 3');
...
assert.equal(newEp.sleepQuality, null, 'sleepQuality null by default');
assert.equal(newEp.alcoholTiming, undefined, 'alcoholTiming absent from new entries');
assert.equal(newEp.caffeineTiming, undefined, 'caffeineTiming absent from new entries');

// new fields can be set via fields argument
const withTriggers = createEpisode({ stress: 'high', position: 'lying', sleepQuality: 'good' });
assert.equal(withTriggers.stress, 'high');
assert.equal(withTriggers.position, 'lying');
assert.equal(withTriggers.sleepQuality, 'good');
...
addEpisode(createEpisode({
  ...
  surroundingNoise: 'quiet', sleepQuality: 'good',
}));
...
assert.ok(!('sleepQuality' in defs), 'sleepQuality not carried forward');
